...
- ✅ Modern browser (Chrome 86+, Edge 86+, Safari 15.2+, Firefox 111+)
- ✅ Familiarity with TypeScript, React, and Next.js App Router
...
git-explorer/
├── app/
│   ├── page.tsx                      # ⭐ Main page (repo selector + treemap display)
│   ├── layout.tsx                    # Root layout
│   ├── globals.css                   # Global styles
│   ├── api/
│   │   └── git-analysis/
│   │       └── route.ts              # ⭐ API: Git analysis endpoint
│   └── components/
│       ├── RepositorySelector.tsx    # ⭐ User input for repo path
│       ├── TreemapChart.tsx          # ⭐ Visx treemap visualization
│       ├── LoadingState.tsx          # Loading spinner
│       └── DateRangeSelector.tsx     # (P2 - not in MVP)
│
├── lib/
│   ├── git/
│   │   ├── analyzer.ts               # ⭐ simple-git wrapper, commit counting
│   │   ├── tree-builder.ts           # ⭐ Build hierarchical tree from files
│   │   └── types.ts                  # ⭐ TypeScript interfaces
│   ├── treemap/
│   │   ├── color-scale.ts            # ⭐ Dark green → light gray gradient
│   │   └── data-transformer.ts       # Transform git data to visx format
│   └── utils/
│       └── date-helpers.ts           # Date range calculations
│
├── __tests__/
...
⭐ = Files you'll create or modify for this feature
```
...
**Run**: `npm test` → Test fails (function doesn't exist) ✅ RED
...
**Run**: `npm test` → Test still passes ✅ REFACTOR
...
- Map scores to colors (dark green → light gray)
...
| Color scale | Sequential (dark green → light gray) | Shows frequency clearly, accessible |
...
2. ✅ Verify environment setup (`npm test`, `npm run dev`)
3. ✅ Review [spec.md](spec.md) to understand user stories
4. ✅ Review [data-model.md](data-model.md) to understand entities
5. → Run `/speckit.tasks` to generate task breakdown
6. → Start implementing tasks following TDD workflow
7. → Mark tasks complete as you go

---

**Remember**: Tests first, always. The constitution is non-negotiable on TDD. 🧪
